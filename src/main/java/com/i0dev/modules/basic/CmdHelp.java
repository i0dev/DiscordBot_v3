...
public class CmdHelp extends DiscordCommand {
    ...
    public static void run(CommandEvent e) {
        ...
        for (BasicCommand registeredCommand : Bot.getRegisteredCommands().stream().filter(command -> !(command instanceof AdvancedCommand)).collect(Collectors.toList())) {
            basicCommands.append("`").append(GeneralConfig.get().getPrefixes().get(0)).append(registeredCommand.getAliases().get(0)).append("`").append(", ");
        }
        ...
        for (BasicCommand registeredCommand : Bot.getRegisteredCommands().stream().filter(command -> (command instanceof AdvancedCommand)).collect(Collectors.toList())) {
            ...
            basicCommands.append("`").append(GeneralConfig.get().getPrefixes().get(0)).append(registeredCommand.getAliases().get(0)).append(" (").append(internal).append(")`").append("\n");
        }
