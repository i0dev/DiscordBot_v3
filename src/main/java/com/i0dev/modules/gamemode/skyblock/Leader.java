...
public class Leader extends SuperDiscordCommand {
    ...
    @CommandData(commandID = "leader", parentClass = SkyblockManager.class, identifier = "SkyBlock Leader", usage = "<user>", messageLength = 2)
    public static void run(CommandEvent e) {
        User user;
        if ((user = FindUtil.getUser(e.getOffsetSplit().get(1), e.getMessage())) == null) return;

        e.reply(EmbedMaker.builder().user(user).embedColor(EmbedColor.SUCCESS).content("You have successfully given {tag} the SkyBlock Leader role.").build());
